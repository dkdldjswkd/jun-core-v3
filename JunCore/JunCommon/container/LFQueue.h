...
template<typename T>
LFQueue<T>::~LFQueue() {
	Node* head = (Node*)(headStamp & kUseBitMask);
...
void LFQueue<T>::Enqueue(T data) {
	...
	for (;;) {
		...
		Node* tailClean = (Node*)(copyTailStamp & kUseBitMask);
		...
		if (tailNext == nullptr) {
			...
			if (InterlockedCompareExchange64((LONG64*)&tailClean->next, (LONG64)enqueNode, NULL) == NULL) {
				...
				// tail �̵�
				DWORD64 newTailStamp = ((copyTailStamp + kStampCount) & kStampMask) | (DWORD64)enqueNode;
			...
			// tail �̵�
			DWORD64 newTailStamp = ((copyTailStamp + kStampCount) & kStampMask) | (DWORD64)tailNext;
...
bool LFQueue<T>::Dequeue(T* data) {
	...
	for (;;) {
		...
		// Dequeue
		//------------------------------
		Node* headClean = (Node*)(copyHeadStamp & kUseBitMask);
		...
		// Dequeue �õ�
		DWORD64 newHeadStamp = ((copyHeadStamp + kStampCount) & kStampMask) | (DWORD64)headNext;
